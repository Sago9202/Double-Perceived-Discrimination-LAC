...
# Collapsed skin color palette
# res0 <- mvord(formula = MMO2(dis11, dis12) ~ 0 + colorr_collapsed + sex + etid, 
...
#               error.structure = cor_equi(~ pais), data = dat_reg, weights.name = "weight1500", link = mvlogit()) 
# resF <- mvord(formula = MMO2(dis11, dis12) ~ 0 + colorr_collapsed + sex + etid + colorr_collapsed:sex + colorr_collapsed:etid + etid:sex + q12cn + q12bn + q2 + I(q2^2) + ur + edre + inc_ran + ocup, 
#               error.structure = cor_equi(~ pais), data = dat_reg, weights.name = "weight1500", link = mvlogit()) 
...
# ggplot(filter(dat_plot[dat_plot$category=="Never",], outcome == "dis12"),  aes(x = colorr, y = predicted_prob, color = sex, group = sex)) +
#   geom_smooth(method = "loess", formula = y ~ x, span = 1.0, se = T, linewidth = 1) + 
#   stat_summary(fun = mean, geom = "point", size = 1.5, alpha = 0.4) +     
#   facet_wrap(etid ~ ., ncol = 3) +  
#   scale_x_continuous(breaks = scales::pretty_breaks(n = 11)) +
#   labs(x = "Skin Color of Respondent", y = "Predicted Probability",color = "Ethnicity") +
#   theme_minimal() + scale_color_manual(values = cal_palette("kelp1")) +
...
ggplot() +
  # Lines for male and female probabilities (without ribbons, for clarity)
  geom_line(data = prob_long, aes(x = colorr, y = prob, color = sex), linewidth = 1.2) +
  
  # Gap line with its OWN standard error (the one you asked about)
  geom_ribbon(data = diff_data, aes(x = colorr, ymin = lower_CI, ymax = upper_CI), 
              fill = "black", alpha = 0.15) +
  geom_line(data = diff_data, aes(x = colorr, y = gender_gap, linetype = "Gender gap"), 
            color = "black", linewidth = 1) +
  
  # Optional: add points with error bars for the gap at each skin tone
  geom_errorbar(data = diff_data, aes(x = colorr, ymin = lower_CI, ymax = upper_CI), 
                width = 0.2, color = "black", alpha = 0.5) +
  geom_point(data = diff_data, aes(x = colorr, y = gender_gap), 
             color = "black", size = 2) +
  ...
  labs(x = "Respondent's Skin Color (1 = Lightest, 11 = Darkest)",
       y = "Probability of reporting 'Many times'") +
  theme_minimal() +
...
ggplot() +
  ...
  geom_ribbon(data = diff_data, aes(x = colorr, ymin = lower_CI, ymax = upper_CI), 
              fill = "black", alpha = 0.15) +
  geom_line(data = diff_data, aes(x = colorr, y = gender_gap, linetype = "Diff"), 
            color = "black", linewidth = 1) +
...
# "#03051AFF"  "#A11A5BFF" "#E83F3FFF"  "#FAEBDDFF"
...
# Merge into cors_coef (which already has pais, cor_pais, sd, etc.)
cors_coef <- cors_coef %>%
...
## -----------------------------------------------------------------
## Reconstruct predicted raw correlation from V1, V2, crosscov, rho
## and compare to the actual raw polychoric correlation
## -----------------------------------------------------------------
full <- stats_df %>%
...
#  geom_smooth(method = "lm", se = FALSE, color = "grey70", linetype = "dashed", size = 0.8) +
